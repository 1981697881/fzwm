...
class PickingReturnSourcingDetail extends StatefulWidget {
  ...
  PickingReturnSourcingDetail({Key? key, @required this.FBillNo}) : super(key: key);

  @override
  _PickingReturnSourcingDetailState createState() => _PickingReturnSourcingDetailState(FBillNo);
}
...
class _PickingReturnSourcingDetailState extends State<PickingReturnSourcingDetail> {
  ...
  _PickingReturnSourcingDetailState(FBillNo) {
    if (FBillNo != null) {
  ...
  getOrderList() async {
    Map<String, dynamic> userMap = Map();
    print(fBillNo);
    userMap['FilterString'] = "FActualQty>FSelPrcdReturnQty and fBillNo='$fBillNo'";
    userMap['FormId'] = 'SUB_PickMtrl';
    userMap['FieldKeys'] =
    'FBillNo,FSubOrgId.FNumber,FSubOrgId.FName,FDate,FEntity_FEntryId,FMaterialId.FNumber,FMaterialId.FName,FMaterialId.FSpecification,FOwnerId.FNumber,FOwnerId.FName,FUnitId.FNumber,FUnitId.FName,FActualQty,FApproveDate,FSelPrcdReturnQty,FID,FSupplierId.FNumber,FSupplierId.FName,FStockID.FName,FStockID.FNumber,FLot.FNumber,F_ora_Assistant.FNumber,FStockID.FIsOpenLocation,FMaterialId.FIsBatchManage,FSubReqBillNo,FKeeperId.FNumber,FParentOwnerId.FNumber';
    Map<String, dynamic> dataMap = Map();
    dataMap['data'] = userMap;
  ...
  saveOrder() async {
    if (this.hobby.length > 0) {
      ...
      Map<String, dynamic> dataMap = Map();
      dataMap['formid'] = 'SUB_RETURNMTRL';
      Map<String, dynamic> orderMap = Map();
      orderMap['NeedReturnFields'] = [];
      orderMap['IsDeleteEntry'] = false;
      Map<String, dynamic> Model = Map();
      Model['FID'] = 0;
      Model['FBillType'] = {"FNUMBER": "WWTLD01_SYS"};
      Model['FDate'] = FDate;
      //获取登录信息
      SharedPreferences sharedPreferences = await SharedPreferences.getInstance();
      var menuData = sharedPreferences.getString('MenuPermissions');
      var deptData = jsonDecode(menuData)[0];
      Model['FSubOrgId'] = {"FNumber": orderDate[0][1].toString()};
      Model['FStockOrgId'] = {"FNumber": orderDate[0][1].toString()};
      Model['FSubSupplierId'] = {"FNumber": orderDate[0][16].toString()};
      var FEntity = [];
      var hobbyIndex = 0;
      this.hobby.forEach((element) {
        if (element[8]['value']['value'] != '0' &&
          ...
          FEntityItem['FStockUnitId'] = {"FNumber": element[6]['value']['value']};
          FEntityItem['FReturnType'] = 1;
          FEntityItem['FSUBReqBillNo'] = orderDate[hobbyIndex][24];
          FEntityItem['FOwnerTypeId'] = "BD_OwnerOrg";
          FEntityItem['FOwnerId'] = {"FNumber": orderDate[hobbyIndex][8]};
          FEntityItem['FKeeperTypeId'] = "BD_KeeperOrg";
          FEntityItem['FKeeperId'] = {"FNumber": orderDate[hobbyIndex][25]};
          FEntityItem['FParentOwnerTypeId'] = "BD_OwnerOrg";
          FEntityItem['FParentOwnerId'] = {"FNumber": orderDate[hobbyIndex][26]};
      ...
      if (res['Result']['ResponseStatus']['IsSuccess']) {
        ...
        //提交
        HandlerOrder.orderHandler(context,submitMap,1,"SUB_RETURNMTRL",SubmitEntity.submit(submitMap)).then((submitResult) {
          if(submitResult){
            //审核
            HandlerOrder.orderHandler(context,submitMap,1,"SUB_RETURNMTRL",SubmitEntity.audit(submitMap)).then((auditResult) {
              if(auditResult){
                //提交清空页面
                setState(() {
                  this.hobby = [];
                ...
                //失败后反审
                HandlerOrder.orderHandler(context,submitMap,0,"SUB_RETURNMTRL",SubmitEntity.unAudit(submitMap)).then((unAuditResult) {
                  if(unAuditResult){
                    this.isSubmit = false;
                  }
                });
